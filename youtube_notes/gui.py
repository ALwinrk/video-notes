...
class YouTubeNotesGUI:
    ...
    def _build_widgets(self) -> None:
        ...
        # API 地址
        ttk.Label(llm_frame, text="API 地址：").grid(row=1, column=0, sticky="w", padx=(0, 4), pady=(4, 0))
        self.api_base_var = tk.StringVar()
        self.api_base_entry = ttk.Entry(
            llm_frame, textvariable=self.api_base_var, font=self._font_mono,
        )
        self.api_base_entry.grid(row=1, column=1, columnspan=2, sticky="ew", padx=(0, 12), pady=(4, 0))

        # API 密钥
        ttk.Label(llm_frame, text="API 密钥：").grid(row=2, column=0, sticky="w", padx=(0, 4), pady=(8, 0))
        self.api_key_var = tk.StringVar()
        self.api_key_entry = ttk.Entry(
            llm_frame, textvariable=self.api_key_var, show="*", font=self._font_mono,
        )
        self.api_key_entry.grid(row=2, column=1, columnspan=2, sticky="ew", padx=(0, 12), pady=(8, 0))
        ...
        ttk.Checkbutton(
            settings_frame, text="年龄限制视频自动从 Chrome 登录",
    ...
    def _on_provider_changed(self) -> None:
        ...
        if info.default_api_base:
            self.api_base_var.set(info.default_api_base)
